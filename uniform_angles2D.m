function [ncells,nxa,mu,mu_b,pw] = uniform_angles2D(N,phi)
...
%   a mesh of the 2-sphere mu_x,mu_y,mu_z \in [-1,1] x [-1,1] x phi
...
%   phi: angle down from the z-axis that fixes the plane of propagation
%   directions. pi/2 for k_z=0
...
% The algorithm discretizes theta \in [0, 2pi ) then generates the boundary
% rays mu_b from these \theta
% 
% The actual sampling rays are generated by averaging the \theta of
...
mu_b = zeros(nxa(1),1,3);
mu = zeros(nxa(1),1,3); %identify with left, top boundaries
ncells= nxa(1);
...
for i=1:nxa(1)-1
    j=1;
    mu(i,j,1) = cos((theta(i) + theta(i+1))./2)*sin(phi);
    mu(i,j,2) = sin((theta(i) + theta(i+1))./2)*sin(phi);
    mu(i,j,3) = cos(phi);
end
...
%ray
mu(N,j,1) = cos((theta(N) + 2*pi)./2)*sin(phi);
mu(N,j,2) = sin((theta(N) + 2*pi)./2)*sin(phi);
mu(N,j,3) = cos(phi);

%------------------CALCULATE QUADRATURE WEIGHTS -------------------
% Should this be proportional to the "size of the cell"?
pw = 1./(ncells)*ones(nxa(1),1);
